...
FlightRadar24, PlaneFinder, plane.watch, sdrmap) do it as part of the feeder
with no separate toggle; OpenSky, ADSBHub, and 1090MHz UK are ADS-B only.
...
- **Per-feeder status** (`ha_feeder_status`) → **one device per enabled feeder**
  ...
  **Aircraft MLAT**: they are not measuring the same thing, and FlightRadar24
  ...
  ADSBHub, plane.watch, sdrmap and 1090MHz UK run clients that expose no status
  endpoint or file, and the community aggregators (adsb.lol, adsb.fi,
  airplanes.live, …) are network connectors with no client process at all — so
  for those there is no per-feeder view to publish, only the Connection and
  Uptime sensors every feeder gets.
...
piaware generates an identity the first time it runs and keeps it on `/data`. If
`/data` is ever lost — a wiped volume, or a reinstall (Home Assistant gives a
new install a fresh `/data`) — it silently registers again as a **brand-new
site**, with no error to notice. Pin it in options instead, and `/data` then
...
With `piaware_feeder_id` set, FlightAware ties the feed to that feeder ID
regardless of what is on `/data`, so a reinstall or a wiped volume keeps the
same site instead of registering a new one.
